...
run('generate_test_srs(2048)', () => {
  ...
  assert(srs && srs.handle && !srs.handle.isNull(), 'SRS handle is null');
});
...
run('proveTest 100 constraints', () => {
  ...
  assert(proof && !proof.handle.isNull(), 'proof handle is null');
...
run('proveTest 500 constraints', () => {
  ...
  assert(!proof.handle.isNull(), 'proof handle is null');
...
const r1csPath = path.resolve(__dirname, '../../examples/circuits/test_circuit.r1cs');
run('proveR1cs test_circuit.r1cs', () => {
  const witness = Array.from({ length: 12 }, () => 1n);
  const proof = proveR1cs(r1csPath, witness, srs);
  assert(!proof.handle.isNull(), 'R1CS proof handle is null');
...
run('aggregateProofs 2 proofs', () => {
  ...
  assert(!batch.handle.isNull(), 'batch handle is null');
